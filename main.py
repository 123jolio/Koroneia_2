...
1. Εισαγωγική Σελίδα (Intro)
2. Επεξεργασία Λίμνης (Lake Processing)
3. Πίνακας Ελέγχου Ποιότητας Νερού (Water Quality Dashboard)
4. Ανάλυση Προτύπων (Pattern Analysis)

Επιπλέον, υπάρχει μια placeholder σελίδα για το "Water Processing".

Έχει επίσης ενσωματωθεί ένα νέο UI με δύο σειρές κουμπιών:
  - Πρώτη σειρά: Επιλογή Δείκτη (π.χ., Πραγματικό, Χλωροφύλλη, Πολυφύτου, Γαλαρία, CDOM, Colour)
  - Δεύτερη σειρά: Είδος Ανάλυσης (Lake Processing, Water Processing, Water Quality Dashboard, Pattern Analysis)

Πριν εκτελέσετε, βεβαιωθείτε ότι:
  - Τα GeoTIFF αρχεία και το shapefile.xml βρίσκονται στον φάκελο "GeoTIFFs".
  - Υπάρχει ένα logo.jpg στον ίδιο φάκελο του script (προαιρετικά).
...
def run_intro_page():
    ...
    st.markdown(
        ...
        **Pattern Analysis:**  
        ...
        Use the new buttons (above) to navigate between the pages and choose the index you want to analyze.
...
def run_lake_processing_app():
    ...
    # -----------------------------------------------------------
    # Additional Analysis: Monthly and Yearly Days in Range
    # -----------------------------------------------------------
    ...
    # Group 1: Monthly Days in Range Analysis (single shared colorbar)
...
def run_water_quality_dashboard():
    ...
    Displays interactive plots, video data, and sampling point analysis.
    """
    import json
    import base64
    import pandas as pd
...
def run_pattern_analysis():
    """
    Analyze spatial and temporal patterns based on monthly days in range data.
    """
...
# -----------------------------------------------------------------------------
# Page Placeholder: Water Processing
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# New: Custom UI (Two-Row Button Matrix)
# -----------------------------------------------------------------------------
def run_custom_ui():
    """
    Draws a two-row "table" of buttons:
      - First row: 'Επιλογή Δείκτη' (Πραγματικό, Χλωροφύλλη, Πολυφύτου, Γαλαρία, CDOM, Colour)
      - Second row: 'Είδος Ανάλυσης' (Lake Processing, Water Processing, Water Quality Dashboard, Pattern Analysis)
    Stores selections in st.session_state.
    """
    st.markdown(
        "<h2 style='text-align: center;'>"
        "Ποιοτικά χαρακτηριστικά Επιφανειακού Ύδατος<br>"
        "με χρήση Εργαλείων Δορυφορικής Τηλεπισκόπησης"
        "</h2>", 
        unsafe_allow_html=True
    )
...
# -----------------------------------------------------------------------------
def main():
    # Draw the custom UI at the top
    run_custom_ui()
    
    # Decide which page to show based on user selections
    ...
    if choice == "Lake Processing":
        # Example: only run Lake Processing if index is "Πραγματικό" or "Πολυφύτου"
